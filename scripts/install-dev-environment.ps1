...
# ── 3а. Xdebug (PHP дебъгер за VS Code) ─────────────────────────────────────

Write-Header "Xdebug (PHP дебъгер за VS Code)"
$phpExeForXdebug = Get-Command "php" -ErrorAction SilentlyContinue
if (-not $phpExeForXdebug) {
    Write-Warn "PHP не е намерен в PATH – Xdebug ще се настрои при следващо стартиране на скрипта."
} else {
    $phpDirXdebug  = Split-Path $phpExeForXdebug.Source
    $phpIniXdebug  = Join-Path $phpDirXdebug "php.ini"
    $extDirXdebug  = Join-Path $phpDirXdebug "ext"
    $xdebugDll     = Join-Path $extDirXdebug "php_xdebug.dll"

    # Проверка дали Xdebug вече е зареден
    $xdebugLoaded = (php -m 2>$null) -contains "xdebug"
    if ($xdebugLoaded) {
        Write-Skip "Xdebug (вече е зареден)"
    } else {
        # Определяме характеристиките на PHP build-а
        $phpShortVer = php -r "echo PHP_MAJOR_VERSION . '.' . PHP_MINOR_VERSION;" 2>$null
        $phpNts      = php -r "echo PHP_ZTS ? 'ts' : 'nts';" 2>$null
        $phpArch     = php -r "echo PHP_INT_SIZE == 8 ? 'x86_64' : 'x86';" 2>$null
        # PHP 8.x NTS Windows се компилира с Visual C++ 2022 = vs17
        $vsVer = "vs17"

        Write-Step "PHP $phpShortVer $phpNts $phpArch – търсене на подходящ Xdebug на xdebug.org ..."

        $xdebugUrl = $null
        try {
            [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12
            $downloadPage = Invoke-WebRequest -Uri "https://xdebug.org/download" -UseBasicParsing
            # Пример на href: '/files/php_xdebug-3.5.1-8.5-nts-vs17-x86_64.dll'
            $dllPattern = "href='(/files/php_xdebug-[\d.]+-${phpShortVer}-${phpNts}-${vsVer}-${phpArch}\.dll)'"
            $dllMatch   = [regex]::Match($downloadPage.Content, $dllPattern)
            if ($dllMatch.Success) {
                $xdebugUrl = "https://xdebug.org" + $dllMatch.Groups[1].Value
            }
        } catch {
            Write-Warn "Не може да се достъпи xdebug.org: $_"
        }

        if (-not $xdebugUrl) {
            Write-Warn "Не намерен Xdebug DLL за PHP $phpShortVer $phpNts $phpArch."
            Write-Warn "Проверете ръчно: https://xdebug.org/wizard"
        } else {
            Write-Step "Изтегляне: $xdebugUrl ..."
            try {
                Invoke-WebRequest -Uri $xdebugUrl -OutFile $xdebugDll -UseBasicParsing
                $sizeKB = [int]((Get-Item $xdebugDll).Length / 1KB)
                Write-OK "php_xdebug.dll изтеглен ($sizeKB KB) -> $xdebugDll"
            } catch {
                Write-Warn "Изтеглянето не успя: $_"
            }
        }

        # Добавяме конфигурация в php.ini (само ако DLL е налице и блокът не съществува)
        if ((Test-Path $xdebugDll) -and (Test-Path $phpIniXdebug)) {
            $iniContent = Get-Content $phpIniXdebug -Raw
            if ($iniContent -notmatch "xdebug\.mode") {
                $xdebugConfig = "`r`n[xdebug]"
                $xdebugConfig += "`r`nzend_extension=php_xdebug"
                $xdebugConfig += "`r`nxdebug.mode=debug"
                $xdebugConfig += "`r`nxdebug.start_with_request=yes"
                $xdebugConfig += "`r`nxdebug.client_host=127.0.0.1"
                $xdebugConfig += "`r`nxdebug.client_port=9003"
                $xdebugConfig += "`r`nxdebug.log_level=0"
                Add-Content -Path $phpIniXdebug -Value $xdebugConfig
                Write-OK "Xdebug конфигуриран в php.ini (mode=debug, порт 9003)."
            } else {
                Write-Skip "Xdebug (вече е конфигуриран в php.ini)"
            }
        }
    }
}
...
Write-Host "  4. PHP Дебъгване в VS Code:"
Write-Host "       - Отворете файл index.php"
Write-Host "       - Сложете breakpoint (клик вляво от номера на ред)"
Write-Host "       - Run & Debug -> изберете 'PHP: XX-example' -> F5"
Write-Host "       (launch.json е в .vscode/ папката на проекта)"
Write-Host "  5. За Spring Boot: използвайте IntelliJ IDEA или VS Code."
Write-Host "  6. Управление на PostgreSQL:"
